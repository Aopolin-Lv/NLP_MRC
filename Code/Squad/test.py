...
def train(train_dataset, model, opt):
    """
    Train the model
    :param train_dataset:
    :param model:
    ...
    optimizer = AdamW(optimizer_grouped_parameters, lr=opt.learning_rate, eps=opt.adam_eposilon)
    scheduler = get_linear_schedule_with_warmup(
        optimizer_grouped_parameters, num_warmup_steps=opt.warmup_steps, num_training_steps=t_total
    )
...
def evaluate(opt, model, tokenizer, prefix=""):
    dataset, examples, features = load_and_cache_example(opt, tokenizer, evaluate=True, output_examples=True)
    ...
    eval_dataloader = DataLoader(dataset, sampler=eval_sampler, batch_size=opt.eval_batch_size)
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        output_prediction_file = os.path.join(opt.output_dir, "predictions_{}.json".format(prefix))
        output_nbest_file = os.path.join(opt.output_dir, "nbest_predictions_{}.json".format(prefix))

        if opt.version_2_with_negative:
            output_null_log_odds_file = os.path.join(opt.output_dir, "null_odds_{}.json".format(prefix))
        else:
        ...
        predictions = compute_predictions_logits(
            ...
            all_results,
            opt.n_best_size,
            opt.max_answer_length,
            opt.do_lower_case,
            output_prediction_file,
            output_nbest_file,
            output_null_log_odds_file,
            opt.verbose_logging,
            opt.version_2_with_negative,
            opt.null_score_diff_threshold,
            tokenizer,
        )
...
def compute_predictions_logits(
    ...
    for feature in all_features:
        example_iondex_to_features[feature.example_index].append(feature)
